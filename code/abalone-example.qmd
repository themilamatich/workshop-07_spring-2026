...
# here shows where to find data when it is in a folder 
...
H~A~: **There is a effect of pH on abalone growth**
...
abalone_clean <- abalone |> 
  ...
  clean_names() |>   
  # selecting columns of interest to show/use
...
# base layer; ggplot
ggplot(data = abalone_clean, # data from cleaned data set
...
abalone_model <- lm(
  change_in_area ~ mean_ph, # formula: change in she;; as a function of pH
  data = abalone_clean      # data frame: abalone clean
)
```
...
'x', 'predictor', 'std.error', 'conf.low', 'group' (not going to worry about 'group')
...
**At pH = 8, teh model prodicts abalone shell growth of 5.36 [95% CI: 5.08, 5.64] mm^2d^-1**
